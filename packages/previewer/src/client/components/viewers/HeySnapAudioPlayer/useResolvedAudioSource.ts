 ...
 * native `<audio>` tag (no fetch needed for the player to function), while
 * `File` / `Blob` / `ArrayBuffer` / `Uint8Array` sources are wrapped in a
 * blob object URL whose lifetime tracks the resolved state.
 */
export type HeySnapAudioSrc = string | File | Blob | ArrayBuffer | Uint8Array;

export interface ResolvedAudioSource {
  /** Value to hand to `<audio src>`. Either the original URL or a blob: URL. */
  url: string;
...
interface State {
  resolved: ResolvedAudioSource | null;
  error: Error | null;
...
/**
 * Normalize the polymorphic `src` into a `{ url, name }` pair an `<audio>` can
 * load. URL strings pass through unchanged; binary sources get an object URL
 * that's revoked when `src` changes or the player unmounts.
 */
export function useResolvedAudioSource(src: HeySnapAudioSrc): State {
 ...
 * so the `<audio>` mounts immediately without a loading flash, but defer
 ...
 */
function initial(src: HeySnapAudioSrc): State {
...
function resolveSync(src: HeySnapAudioSrc): ResolvedAudioSource {
  if (typeof src === "string") {
  ...
  if (src instanceof ArrayBuffer) {
    ...
    return { url: URL.createObjectURL(blob), name: "audio", isObjectUrl: true };
  }
  ...
  throw new Error(
    "HeySnapAudioPlayer: `src` must be a URL string, File, Blob, ArrayBuffer, or Uint8Array.",
  );
}

/**
 * Pull a human-readable filename out of a URL. Falls back to "audio" for
 * opaque sources (data URLs, blob URLs, weird inputs) so the title is never
 * blank.
 */
function filenameFromUrl(src: string): string {
  if (src.startsWith("data:") || src.startsWith("blob:")) return "audio";
  try {
    const base = typeof window !== "undefined" ? window.location?.href : "https://localhost/";
    const url = new URL(src, base);
    const last = url.pathname.split("/").filter(Boolean).pop();
    return last ? decodeURIComponent(last) : "audio";
  } catch {
