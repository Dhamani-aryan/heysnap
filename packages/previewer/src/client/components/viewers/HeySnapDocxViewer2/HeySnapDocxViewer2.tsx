...
import {
  useResolvedDocxSource,
  type HeySnapDocxSrc,
} from "../HeySnapDocxViewer/useResolvedDocxSource";

export type { HeySnapDocxSrc } from "../HeySnapDocxViewer/useResolvedDocxSource";

export interface HeySnapDocxViewer2Props extends Omit<BaseViewerProps, "src"> {
  src: HeySnapDocxSrc;
  documentName?: string;
  licenseKey?: string;
...
const rootClass = (extra?: string) =>
  ["heysnap-viewer", "heysnap-viewer--docx2", extra].filter(Boolean).join(" ");
...
export function HeySnapDocxViewer2({
  src,
  className,
  style,
  documentName,
  ...
  onReady,
  onError,
}: HeySnapDocxViewer2Props): React.ReactElement {
  const { resolved, error: resolveError } = useResolvedDocxSource(src);
  ...
  useEffect(() => {
    ...
    const loadDocument = async (): Promise<void> => {
      try {
        ...
        const file = new File([new Uint8Array(resolved.buffer)], title, {
          type: DOCX_MIME,
        });
    ...
    >
      ...
      >
        ...
        {viewerError !== null ? (
          ...
          >
            Failed to load document: {viewerError.message}
          </p>
        ) : null}
      </div>
    </div>
