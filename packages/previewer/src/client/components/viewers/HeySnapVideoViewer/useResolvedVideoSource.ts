 ...
 * native `<video>` tag (no fetch needed for the viewer to function), while
 * `File` / `Blob` / `ArrayBuffer` / `Uint8Array` sources are wrapped in a
 * blob object URL whose lifetime tracks the resolved state.
 */
export type HeySnapVideoSrc = string | File | Blob | ArrayBuffer | Uint8Array;

export interface ResolvedVideoSource {
  /** Value to hand to `<video src>`. Either the original URL or a blob: URL. */
  url: string;
...
interface State {
  resolved: ResolvedVideoSource | null;
  error: Error | null;
...
/**
 * Normalize the polymorphic `src` into a `{ url, name }` pair a `<video>` can
 ...
 */
export function useResolvedVideoSource(src: HeySnapVideoSrc): State {
 ...
 * so the `<video>` mounts immediately without a loading flash, but defer
 ...
 */
function initial(src: HeySnapVideoSrc): State {
...
function resolveSync(src: HeySnapVideoSrc): ResolvedVideoSource {
  if (typeof src === "string") {
  ...
  if (src instanceof ArrayBuffer) {
    ...
    return { url: URL.createObjectURL(blob), name: "video", isObjectUrl: true };
  }
  ...
  throw new Error(
    "HeySnapVideoViewer: `src` must be a URL string, File, Blob, ArrayBuffer, or Uint8Array.",
  );
}

/**
 * Pull a human-readable filename out of a URL. Falls back to "video" for
 * opaque sources (data URLs, blob URLs, weird inputs) so the title is never
 * blank.
 */
function filenameFromUrl(src: string): string {
  if (src.startsWith("data:") || src.startsWith("blob:")) return "video";
  try {
