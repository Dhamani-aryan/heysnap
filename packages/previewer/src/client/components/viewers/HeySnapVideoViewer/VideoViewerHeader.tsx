...
const VIDEO_ZOOM_PRESETS = [0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0, 3.0, 4.0, 6.0, 8.0] as const;
...
function getPreviousZoomPreset(current: number): number {
  for (let i = VIDEO_ZOOM_PRESETS.length - 1; i >= 0; i--) {
    if (VIDEO_ZOOM_PRESETS[i] < current - 1e-3) return VIDEO_ZOOM_PRESETS[i];
  }
  return MIN_VIDEO_ZOOM;
}

function getNextZoomPreset(current: number): number {
  for (const p of VIDEO_ZOOM_PRESETS) {
    if (p > current + 1e-3) return p;
  }
  return MAX_VIDEO_ZOOM;
}
...
const headerBaseStyle: CSSProperties = {
  ...
  // image / PDF / DOCX / XLSX headers — one toolbar idiom across viewers.
  gap: 28,
  height: 40,
  flexShrink: 0,
  padding: "0 8px",
...
/**
 * Stateless visual shell. Renders before the video has loaded so the
 ...
 */
export function VideoTitle({ name, style }: VideoTitleProps) {
  return (
    <div
      style={{
        display: "flex",
...
export function VideoZoomControls({ zoom, onZoom, disabled = false }: VideoZoomControlsProps) {
  const percent = Math.round(zoom * 100);
  const canZoomOut = !disabled && zoom > MIN_VIDEO_ZOOM + 1e-3;
  const canZoomIn = !disabled && zoom < MAX_VIDEO_ZOOM - 1e-3;
...
/**
 * Downloads the displayed video. Object-URL sources (File / Blob / buffer)
 ...
 * can't paper over.
 */
export function VideoDownloadButton({ resolved }: VideoDownloadButtonProps) {
  return (
    <IconButton aria-label="Download" onClick={() => downloadVideo(resolved)}>
      <HugeiconsIcon icon={Download01Icon} size={17} strokeWidth={1.8} />
    </IconButton>
  );
}

function downloadVideo(resolved: ResolvedVideoSource) {
