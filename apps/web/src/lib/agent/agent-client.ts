...
export async function getAgentThread(
  ...
  return fetchJson<GetAgentThreadResult>(
    buildAgentUrl(agentBaseUrl, `/threads/${encodeURIComponent(threadId)}`),
  )
}
...
function createRunHandle(state: RunStreamState): AgentRunHandle {
  return {
    done: state.done,
    cancel: () => {
...
async function runStreamLoop(
  ...
  initialRequest: RunStreamRequest,
): Promise<void> {
  ...
  while (!state.closed && !state.ended) {
    try {
      ...
      if (reconnectAttempts > MAX_RECONNECT_ATTEMPTS) {
        failRun(
          state,
          error instanceof Error ? error : new Error('Agent run stream failed.'),
        )
        return
      }
...
async function sendCancelIfReady(state: RunStreamState): Promise<void> {
  if (
  ...
  try {
    ...
    state.callbacks.onError?.(
      error instanceof Error ? error : new Error('Failed to cancel agent run.'),
    )
  }
}

function failRun(state: RunStreamState, error: Error): void {
  if (state.ended || state.closed) {
    return
  }
...
function settleRun(state: RunStreamState, error?: Error): void {
  if (state.settled) {
    return
  }
...
function parseSseMessage(rawEvent: string): AgentSseMessage | null {
  let id = 0
  let event = ''
...
function isKnownSseEvent(event: string): event is AgentSseMessage['event'] {
  return (
    event === 'run_start' ||
    event === 'event' ||
    event === 'run_end' ||
